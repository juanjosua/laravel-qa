...
<div class="d-flex flex-column vote-controls">
  <a title="This {{ $name }} is useful"
    ...
    onclick="event.preventDefault(); document.getElementById('up-vote-{{ $name }}-{{ $formId }}').submit();">
    <i class="fas fa-caret-up fa-3x"></i>
  </a>
  <form id="up-vote-{{ $name }}-{{ $formId }}" action="{{ $formAction }}" method="post" style="display:none;">
    @csrf
